...
- Web terminal: `/terminal` page serves xterm.js UI that connects to Claude tmux sessions via WebSocket (`flask-sock`). Auth via bearer token (`~/.pieoffice-terminal-token`). `caffeinate` keeps Mac awake during active terminal sessions. Setup: `./scripts/setup-terminal.sh`.
