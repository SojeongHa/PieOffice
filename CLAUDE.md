...
- Web terminal: `/terminal` page serves xterm.js UI that connects to Claude tmux sessions via WebSocket (`flask-sock`). Auth via mTLS (device certificate) + session tokens (for WebSocket). `caffeinate` keeps Mac awake during active terminal sessions. Setup: `./scripts/setup-terminal.sh`.
