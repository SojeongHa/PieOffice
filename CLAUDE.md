...
- Web terminal: asyncio+websockets+pty server on port 10316 (separate process, NOT Flask thread). Auth via mTLS (CERT_OPTIONAL) + session tokens. `caffeinate` keeps Mac awake during active terminal sessions AND pending instance alerts (separate `CaffeinateManager` in app.py, `_sync_alert_caffeinate()` acquires/releases based on `count_pending_alerts()`). Setup: `./scripts/setup-terminal.sh`. NEVER use pty in Flask threads (segfault). NEVER use tmux resize-window -A (locks manual size). Rate limiting: IP-based sliding window (HTTP 30req/min, WS 10conn/min) as defense-in-depth. `setup-terminal.sh` auto-detects Tailscale IP (with userspace socket fallback) for cert SAN; use `--regen-server` to refresh server cert when IPs change. `terminal.html` uses no inline event handlers (CSP `script-src 'self'` compliant); all handlers registered via `addEventListener` in `terminal-client.js`. Terminal server `/sessions` endpoint merges Pie Office alert state (via `/alerts` proxy) into each session entry (`alert_type`, `alert_message`).
