"""Integration tests for terminal server routes (mTLS + session token)."""
...
@pytest.fixture
def app_client():
    from terminal_server import terminal_app

    terminal_app.config["TESTING"] = True
    return terminal_app.test_client()


@pytest.fixture
def session_token(app_client):
    """Acquire a session token (simulates mTLS-authenticated request)."""
    resp = app_client.post("/session-token")
